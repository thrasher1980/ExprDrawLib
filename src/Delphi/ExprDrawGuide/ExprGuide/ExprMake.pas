unit ExprMake;
 ...
 interface
{$LongStrings Off}
  uses ExprDraw,SysUtils,ExpressionException;
   ...
   type TExprBuilder=class
                      protected
                       ...
                       P:Integer;
                       S:AnsiString;
                       function Preprocess(S:string):AnsiString;
                       ...
                       function FuncName(N:AnsiString;var Flags:Integer;Brackets:Boolean):TExprClass;
                       function Token(N:AnsiString;var Flags:Integer):TExprClass;
                       function GreekLetter(N:AnsiString):Integer;
                       function Comma:Boolean;
                       procedure LookForComma;
                      public
                       FuncAutoIndex,VarAutoIndex,PostSymbols:Boolean;
                       constructor Create;
                       function BuildExpr(Expr:AnsiString):TExprClass;
                       function SafeBuildExpr(Expr:AnsiString):TExprClass;
                     end;
     ...
     begin
      ...
      case S[P] of
       ...
       '#':begin
            ...
            raise EExpressionException.Create(EC_ExpectedSymbol,'Ожидается цифра в позиции '+IntToStr(P), P);
...
//             raise EIncorrectExpr.Create('Ожидается "]" в позиции '+IntToStr(P))
             raise EExpressionException.Create(EC_ExpectedSymbol,P,']');
           end;
       '{':begin
            Inc(P);
...
//        raise EIncorrectExpr.Create('Недопустимый символ в позиции '+IntToStr(P))
                 raise EExpressionException.Create(EC_InvalidSymbol,P);
      end;
      if PostSymbols then
       while S[P] in ['^','_','!','`'] do
        case S[P] of
...
//                                    raise EIncorrectExpr.Create('Незавершённая строка');
                              raise EExpressionException.Create(EC_IncompletedString);
                            until S[P]='"';
                            Inc(P)
                           end
                          else
...
//                             raise EIncorrectExpr.Create('Ожидается знак закрывающей скобки в позиции '+IntToStr(P))
      ...
      if M='NUM' then begin
                       ...
                       if B=1 then
                        ...
                        raise EExpressionException.Create(EC_ExpectedDigitals,P);
                       J:=B;
